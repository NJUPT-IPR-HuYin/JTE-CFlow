...
class JTECFlowModel(BaseModel):
    def __init__(self, opt, step):
        super(JTECFlowModel, self).__init__(opt)
        self.opt = opt
    ...
    def optimize_parameters(self, step):
        ...
        if weight_fl > 0:
            if self.opt['optimize_all_z']:
                ...
                epses = None
            # total_loss = 0
            z, nll, y_logits = self.netG(gt=self.real_H, lr=self.var_L, zc=self.zero_channel, reverse=False,
                                         epses=epses,
                                         add_gt_noise=True)
        ...
        if weight_l1 > 0:
            ...
            sr, logdet = self.netG(lr=self.var_L, zc=self.zero_channel, z=z.to(self.var_L.device), eps_std=0,
                                   reverse=True,
                                   add_gt_noise=True)
        ...
        if self.heats is not None:
            for heat in self.heats:
                for i in range(self.n_sample):
                    ...
                    with torch.no_grad():
                        self.fake_H[(heat, i)], logdet = self.netG(lr=self.var_L, zc=self.zero_channel, z=z,
            ...
            with torch.no_grad():
                ...
                self.fake_H[(0, 0)], logdet = self.netG(lr=self.var_L, zc=self.zero_channel, z=z.to(self.var_L.device),
    ...
    def get_sr_with_z(self, lq, zero_channel=None, heat=None, seed=None, z=None, epses=None):
        ...
        with torch.no_grad():
            sr, logdet = self.netG(lr=lq, zc=zero_channel, z=z.cuda(), eps_std=heat, reverse=True, add_gt_noise=True)
        self.netG.train()
        return sr, z
