...
from models_zero.archs.low_light_transformer import low_light_transformer


class JTECFlow(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, scale=4, K=None, opt=None, step=None):
        super(JTECFlow, self).__init__()
        ...
        if opt['cond_encoder'] == 'ConEncoder1':
            self.RRDB = low_light_transformer(nf=64, nframes=5, groups=8, front_RBs=6, back_RBs=0,
                                              center=False, predeblur=True, HR_in=True, w_TSA=True)
    ...
    # @autocast()
    def forward(self, gt=None, lr=None, zc=None, z=None, eps_std=None, reverse=False, epses=None,
        ...
        if not reverse:
            ...
            return self.normal_flow(gt, lr, zc, epses=epses, lr_enc=lr_enc, add_gt_noise=add_gt_noise, step=step,
            ...
            if reverse_with_grad:
                return self.reverse_flow(lr, zc, z, y_onehot=y_label, eps_std=eps_std, epses=epses, lr_enc=lr_enc,
    ...
    def normal_flow(self, gt, lr, zc, y_onehot=None, epses=None, lr_enc=None, add_gt_noise=True, step=None,
                    align_condition_feature=False):
        if self.opt['to_yuv']:
            gt = self.rgb2yuv(gt)
        if lr_enc is None and self.RRDB:
            lr_enc = self.rrdbPreprocessing(lr, zc)
        ...
        #         mean = squeeze2d(lr_enc['color_map'], 8)
        #     else:
    ...
    def reverse_flow(self, lr, zc, z, y_onehot, eps_std, epses=None, lr_enc=None, add_gt_noise=True):
