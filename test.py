...
def main():
    ...
    parser.add_argument("--opt", default="confs/JTECFlow.yml")
    ...
    hr_paths = fiFindByWildcard(os.path.join(hr_dir, '*.*'))
    # lr_paths.sort(key=lambda x: int(x.split(".*")[0].split("\\")[1]))
    # hr_paths.sort(key=lambda x: int(x.split(".*")[0].split("\\")[1]))
    ...
    for lr_path, hr_path, idx_test in zip(lr_paths, hr_paths, range(len(lr_paths))):
        ...
        # Please note that our method JTE-CFlow does not finetune the overall brightness like LLFlow and Kind.#

        # LLFlow follow a similar way of 'Kind' to finetune the overall brightness as illustrated in Line 73 (https://github.com/zhangyhuaee/KinD/blob/master/evaluate_LOLdataset.py).
